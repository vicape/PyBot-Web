-- ──────────────────────────────────────────────────────────────────────────
-- Migración 016 — Roster pendiente de Classroom (grisados hasta login).
-- IDEMPOTENTE. No crea auth.users falsos.
-- ──────────────────────────────────────────────────────────────────────────
...
-- Extender sync Classroom: enrolled + pending
...
begin
  ...
  if p_enrolled is not null and jsonb_typeof(p_enrolled) = 'array' then
    ...
    loop
      ...
      -- Ya activo: sacar de pendientes
      delete from public.course_roster_pending
      where course_id = p_course_id
        and (
          (v_classroom_user_id is not null and classroom_user_id = v_classroom_user_id)
          or (v_classroom_email is not null and lower(email) = lower(v_classroom_email))
        );
  ...
  -- Remover course_members classroom ausentes del roster
  ...
  -- Upsert pendientes (sin cuenta PyBot)
  if p_pending is not null and jsonb_typeof(p_pending) = 'array' then
    ...
    loop
      ...
      values (
        p_course_id, p_org_id, v_classroom_user_id, v_classroom_email, v_display_name, now()
      )
  ...
  -- Limpiar pendientes que ya no están en Classroom
  delete from public.course_roster_pending p
  where p.course_id = p_course_id
    and p.classroom_user_id is not null
    and not (p.classroom_user_id = any (coalesce(p_active_classroom_user_ids, array[]::text[])));
...
-- Compat: overload vieja (4 args) delega a la nueva
create or replace function public.sync_classroom_course_roster(
  p_course_id uuid,
  p_org_id uuid,
  p_enrolled jsonb,
...
as $$
begin
  return public.sync_classroom_course_roster(
    p_course_id,
    p_org_id,
    p_enrolled,
    p_active_classroom_user_ids,
    '[]'::jsonb
  );
end;
$$;
...
-- Al login: activar pendientes cuyo email coincide con el perfil
